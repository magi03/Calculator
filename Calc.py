...
class Calculator:
    def __init__(self):
        ...
        self.window.title("MS_Standard_Calculator")
    ...
    def bind_keys(self):
        ...
        self.window.bind("<Control-m>", lambda event: self.add_to_memory())
        self.window.bind("<Control-Shift-m>", lambda event: self.sub_from_memory())

    def create_special_buttons(self):
        self.create_clear_button()
        self.create_clear_one_button()
    ...
    def create_memory_buttons(self):
        ...
        self.create_recall_button()
        self.create_madd_button()
        self.create_msub_button()
        self.create_mclear_button()
    ...
    def store_memory(self):
        self.memory = self.total_expression

    def create_store_button(self):
        button = tk.Button(self.buttons_frame, text="MS", bg=OFF_WHITE, fg=LABEL_COLOR, font=DEFAULT_FONT_STYLE,
                           borderwidth=0, command=self.store_memory)
        button.grid(row=4, column=5, sticky=tk.NSEW)

    def recall_memory(self):
        self.current_expression = self.memory
        self.update_label()

    def create_recall_button(self):
        button = tk.Button(self.buttons_frame, text="MR", bg=OFF_WHITE, fg=LABEL_COLOR, font=DEFAULT_FONT_STYLE,
                           borderwidth=0, command=self.recall_memory)
        button.grid(row=3, column=5, sticky=tk.NSEW)

    def add_to_memory(self):
        self.memory = (f'{self.total_expression}+{self.current_expression}')

    def create_madd_button(self):
    ...
    def sub_from_memory(self):
        self.memory = (f'{self.total_expression}-{self.current_expression}')

    def create_msub_button(self):
        button = tk.Button(self.buttons_frame, text="M-", bg=OFF_WHITE, fg=LABEL_COLOR, font=DEFAULT_FONT_STYLE,
                           borderwidth=0, command=self.sub_from_memory)
